...
def get_agent_name(agent: AgentBase) -> str:
    """安全获取 AgentScope 智能体名称。"""
    name = getattr(agent, "name", None)
    return name if isinstance(name, str) else agent.__class__.__name__


def format_player_list(
    players: Sequence[AgentBase],
    ...
    if show_roles:
        return "、".join([
            f"{get_agent_name(p)}({getattr(p, 'role', '未知')})"
            for p in players
        ])
    else:
        return "、".join(get_agent_name(p) for p in players)


def majority_vote_cn(
    votes: Mapping[str, Optional[str]],
) -> tuple[Optional[str], int]:
    """中文版多数投票统计"""
    if not votes:
        return None, 0
    
    vote_counts = Counter(vote for vote in votes.values() if vote)
    if not vote_counts:
        return None, 0
...
def check_winning_cn(
    alive_players: Sequence[AgentBase],
    roles: Mapping[str, str],
) -> Optional[str]:
    """检查中文版游戏胜利条件"""
    alive_roles = [roles.get(get_agent_name(p), "村民") for p in alive_players]
...
class GameModerator(AgentBase):
    ...
    async def private_announce(self, content: str) -> Msg:
        """创建只交给指定智能体的系统消息，不打印也不写入公共日志。"""
        return Msg(
            name=self.name,
            content=content,
            role="system",

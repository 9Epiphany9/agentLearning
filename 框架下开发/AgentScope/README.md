...
在项目根目录创建或修改 `.env`，配置 OpenAI 兼容模型：
...
例如 DeepSeek 可将模型设为 `deepseek-chat`，服务地址设为
`https://api.deepseek.com`。程序始终从当前项目的 `.env` 加载这三项配置，
并根据服务地址自动选择 DeepSeek 或 OpenAI 兼容的多人消息格式。
...
### Q: 游戏无法启动？
A: 检查以下几点：
- 确认 `.env` 中的 `LLM_MODEL_ID`、`LLM_API_KEY` 和 `LLM_BASE_URL` 均已设置
- 验证 API Key 是否有效
- 检查网络连接是否正常
